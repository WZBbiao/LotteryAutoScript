...
// 此处填你的飞书机器人webhook地址（例如：https://open.feishu.cn/open-apis/bot/v2/hook/xxxxxxxxxx）
let FEISHU_WEBHOOK = '';
...
function feishuBotNotify(text, desp) {
    return new Promise(resolve => {
        if (FEISHU_WEBHOOK) {
            send({
                method: 'POST',
                ...
                success: res => {
                    try {
                        ...
                        if (data.code === 0) {
                            ...
                            log.error('发送通知', `飞书机器人发送失败: ${data.msg || JSON.stringify(data)}`);
                        }
                    } catch (e) {
                        log.error('发送通知', e);
                    } finally {
            ...
            log.debug('发送通知', '您未提供飞书机器人推送所需的FEISHU_WEBHOOK，取消飞书推送消息通知');
            resolve();
        }
    });
}
